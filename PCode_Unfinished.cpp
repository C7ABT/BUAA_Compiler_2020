//
// Created by Conno on 2020/11/16.
//
...
bool _char(Sym_list & symList, Sym_print & symPrint, Sym_stack & symStack, Sym_symbol & symSymbol) {
    if (isParticular_S_List(symList, "CHARCON")) {
        // 字符
        slist_0_sprint(symList, symPrint);
        slist_1_sprint(symList, symPrint, "<字符>");
        return true;
    }
    else {
        return false;
        ...
        if (isParticular_S_List(symList, "LPARENT")) {
            ...
            if (_table_parameter_value(symList, symPrint, symStack, symSymbol)) {
                ...
                if (isParticular_S_List(symList, "RPARENT")) {
                    // )
                    slist_0_sprint(symList, symPrint);
                    slist_1_sprint(symList, symPrint, "<有返回值函数调用语句>");
                    return true;
                }
                else {
                    retract(3);
        ...
        if (isParticular_S_List(symList, "LPARENT")) {
            ...
            if (_table_parameter_value(symList, symPrint, symStack, symSymbol)) {
                ...
                if (isParticular_S_List(symList, "RPARENT")) {
                    // )
                    slist_0_sprint(symList, symPrint);
                    slist_1_sprint(symList, symPrint, "<无返回值函数调用语句>");
                    return true;
                }
                else {
                    retract(3);
...
bool _statement(Sym_list & symList, Sym_print & symPrint, Sym_stack & symStack, Sym_symbol & symSymbol) {
    ...
    if (_switch(symList, symPrint, symStack, symSymbol)) {
        // 情况语句
        slist_1_sprint(symList, symPrint, "<语句>");
        return true;
    }
    ...
    if (isParticular_S_List(symList, "DEFAULTTK")) {
        ...
        if (isParticular_S_List(symList, "COLON")) {
            ...
            if (_statement(symList, symPrint, symStack, symSymbol)) {
                // 语句
                slist_0_sprint(symList, symPrint);
                slist_1_sprint(symList, symPrint, "<缺省>");
                return true;
            }
            else {
                retract(2);
...
bool _switch(Sym_list & symList, Sym_print & symPrint, Sym_stack & symStack, Sym_symbol & symSymbol) {
    ...
    if (isParticular_S_List(symList, "SWITCHTK")) {
        // switch
        slist_0_sprint(symList, symPrint);
        if (isParticular_S_List(symList, "LPARENT")) {
            // (
            ...
            if (_expression(symList, symPrint, symStack, symSymbol)) {
                ...
                if (isParticular_S_List(symList, "RPARENT")) {
                    // )
                    slist_0_sprint(symList, symPrint);
                    if (isParticular_S_List(symList, "LBRACE")) {
                        // {
                        slist_0_sprint(symList, symPrint);
                        if (_table_cases(symList, symPrint, symStack, symSymbol)) {
                            // 情况表
                            if (_default(symList, symPrint, symStack, symSymbol)) {
                                // 缺省
                                if (isParticular_S_List(symList, "RBRACE")) {
                                    // }
                                    slist_0_sprint(symList, symPrint);
                                    slist_1_sprint(symList, symPrint, "<情况语句>");
                                    return true;
                                }
                                else {
                                    retract(7);
...
bool _case(Sym_list & symList, Sym_print & symPrint, Sym_stack & symStack, Sym_symbol & symSymbol) {
    // ＜情况子语句＞  ::=  case＜常量＞：＜语句＞
    if (isParticular_S_List(symList, "CASETK")) {
        // case
        slist_0_sprint(symList, symPrint);
        if (_const(symList, symPrint, symStack, symSymbol)) {
            // <常量>
            if (isParticular_S_List(symList, "COLON")) {
                // :
                slist_0_sprint(symList, symPrint);
                if (_statement(symList, symPrint, symStack, symSymbol)) {
                    // <语句>
                    slist_1_sprint(symList, symPrint, "<情况子语句>");
                    return true;
                }
                else {
                    retract(3);
...
bool _table_cases(Sym_list & symList, Sym_print & symPrint, Sym_stack & symStack, Sym_symbol & symSymbol) {
    // ＜情况表＞   ::=  ＜情况子语句＞{＜情况子语句＞}
    if (_case(symList, symPrint, symStack, symSymbol)) {
        // 情况子语句
        while (_case(symList, symPrint, symStack, symSymbol)) {
            // 情况子语句
        }
        return true;
    }
...
bool _const(Sym_list & symList, Sym_print & symPrint, Sym_stack & symStack, Sym_symbol & symSymbol) {
    ...
    //             |  ＜字符＞
    if (_int(symList, symPrint, symStack, symSymbol)
        || _char(symList, symPrint, symStack, symSymbol)) {
        // 整数 字符
        slist_1_sprint(symList, symPrint, "<常量>");
        return true;
    }
    else {
        return false;
...
bool _var_define(Sym_list & symList, Sym_print & symPrint, Sym_stack & symStack, Sym_symbol & symSymbol) {
    ...
    //                      {''{'＜常量＞{,＜常量＞}'}'{, '{'＜常量＞{,＜常量＞}'}'}'}'
    if (isParticular_S_List(symList, "INTTK") || isParticular_S_List(symList, "CHARTK")) {
        // 类型标识符
        int i = pos_S_list;
        while (symList.type[i] != "COMMA"
               && symList.type[i] != "SEMICN"
               && symList.type[i] != "ASSIGN" ) {
            i += 1;
        }
        if (symList.type[i] == "COMMA"
            || symList.type[i] == "SEMICN") { // ; ,
            if (_var_define_no_initialization(symList, symPrint, symStack, symSymbol))  {
                slist_1_sprint(symList, symPrint, "<变量定义>");
                return true;
                // 变量定义无初始化
            }
        }
        else {
            if (_var_define_with_initialization(symList, symPrint, symStack, symSymbol))  {
                slist_1_sprint(symList, symPrint, "<变量定义>");
                return true;
                // 变量定义及初始化
            }
    ...
    if (isParticular_S_List(symList, "IDENFR") && symList.type[pos_S_list + 1] == "ASSIGN") {
        ...
        if (isParticular_S_List(symList, "ASSIGN")) {
            ...
            if (_expression(symList, symPrint, symStack, symSymbol)) {
                // 表达式
                emit_op2(10, temp);
                slist_1_sprint(symList, symPrint, "<赋值语句>");
                return true;
            }
            else {
                retract(2);
                    ...
                    if (!isParticular_S_List(symList, "LBRACK")) {
                        ...
                        if (_expression(symList, symPrint, symStack, symSymbol)) {
                            ...
                            if (isParticular_S_List(symList, "RBRACK")) {
                                // ]
                                slist_0_sprint(symList, symPrint);
                                slist_1_sprint(symList, symPrint, "<因子>");
                                return true;
                            }
                            else {
                                retract(4);
...
bool _var_define_no_initialization(Sym_list & symList, Sym_print & symPrint, Sym_stack & symStack, Sym_symbol & symSymbol) {
    ...
    int temp;
    if ((isParticular_S_List(symList, "INTTK") || isParticular_S_List(symList, "CHARTK"))
        && symList.type[pos_S_list + 2] != "LBRACK") {
        temp = isParticular_S_List(symList, "INTTK") ? 1 : 2;
        slist_0_sprint(symList, symPrint);
        if (isParticular_S_List(symList, "IDENFR")
            && symList.type[pos_S_list + 1] != "LBRACK") {
            // 标识符
            symSymbol.name[pos_S_symbol] = symList.name[pos_S_list];
            symSymbol.addr[pos_S_symbol] = pos_stack;
            symSymbol.level[pos_S_symbol] = (func) ? 1 : 0;
            symSymbol.kind[pos_S_symbol] = 2;   // 变量
            symSymbol.type[pos_S_symbol] = temp;
            symSymbol.ref[pos_S_symbol] = 0;
            pos_S_symbol += 1;
            // 入栈
            symStack.name[pos_stack] = symList.name[pos_S_list];
            symStack.type[pos_stack] = temp;
            pos_stack += 1;
            slist_0_sprint(symList, symPrint);
            if (isParticular_S_List(symList, "LBRACK")) {
                // 一维数组 无二维数组！！！！！！！！！！！
                ...
                if (_unsigned_int(symList, symPrint, symStack, symSymbol)) {
                    ...
                    if (isParticular_S_List(symList, "RBRACK")) {
                        ...
                        if (isParticular_S_List(symList, "COMMA")) {
                            ...
                            slist_1_sprint(symList, symPrint, "<变量定义无初始化>");
                            return true;
                        }
                    }
                    else {
