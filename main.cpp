...
#include<string.h>
#include<ctype.h>
char name[20][20] = {"const","int","char","void","main","if","else","switch","case","default","while","for","scanf","printf","return"};
char code[20][20] = {"CONSTTK","INTTK","CHARTK","VOIDTK","MAINTK","IFTK","ELSETK","SWITCHTK","CASETK","DEFAULTTK","WHILETK","FORTK","SCANFTK","PRINTFTK","RETURNTK"};
char head[100][255];
...
{
    char word[255];//常量/变量名
    int type;//常量/变量类型编号
    int func;//表示符号在何处定义
    int dim;//值的维数
    int i, j;//存储数组的下标
    int value[30];//存储常量\变量值
}   user3;

typedef struct func
{
    char name[255];//声明的函数名
    int type;//函数的返回类型
    int num;//参数个数
    char id[255];//参数类型标记
    int loc;//函数开始位置
...
{
    if((letter>=65 && letter <=90) || (letter>=97 && letter <=122) || letter == 95)
        return 1;
    else
        return 0;
}
...
char search_nametab()//查全局\局部表返回数据类型
{
    int i = 0, j = 0;
    char sym[255] = {};
    char tab[255] = {};
...
//    fopen_initialize();
    while(fgets(buff, 255, fp_in) != NULL)
    {
        ...
        for(i=0; i<len; i++, pin=0, memset(temp, 0, sizeof(temp)))
        {
            ...
            else if(isdigit(token))
            {
                ...
                token = buff[i];
                while(token != '\''){
                    if((buff[i]<48||buff[i]>57)&&(buff[i]<65||buff[i]>90)&&(buff[i]<97||buff[i]>122)&&buff[i]!=42&&buff[i]!=43&&buff[i]!=45&&buff[i]!=47&&buff[i]!=95)
                        flag=1;
                    temp[pin++] = buff[i++];
                    token = buff[i];
                }
                ...
                if(token == '\"') flag = 1;
                while(token != '\"'){
                    if((buff[i]<35||buff[i]>126)&&buff[i]!=33&&buff[i]!=32)
                        flag=1;
                    temp[pin++] = buff[i++];
                    token = buff[i];
                }
    ...
    for(i=0; i<gt_num; i++)//全局表测试
    {
        ...
        {
            if(global_tab[i].type==1||global_tab[i].type==3)
                printf("%s %d %d %d\n",global_tab[i].word,global_tab[i].type,global_tab[i].func,global_tab[i].value[0]);
            else if(global_tab[i].type==2||global_tab[i].type==4)
                printf("%s %d %d %c\n",global_tab[i].word,global_tab[i].type,global_tab[i].func,global_tab[i].value[0]);
            else
...
void program()//<程序>::=［<常量说明>］［<变量说明>］{<有返回值函数定义>|<无返回值函数定义>}<主函数>
{
    ...
    if((strcmp(store[now].type,"INTTK") == 0 || strcmp(store[now].type,"CHARTK") == 0) && strcmp(store[now+2].type,"LPARENT") != 0)
    {
        variable_description();//变量说明
    }
    run = 0;
    while((strcmp(store[now].type,"INTTK") == 0 || strcmp(store[now].type,"CHARTK") == 0) || (strcmp(store[now].type,"VOIDTK") == 0 && strcmp(store[now+1].type,"IDENFR") == 0))
    {
        if((strcmp(store[now].type,"INTTK") == 0 || strcmp(store[now].type,"CHARTK") == 0))
        {
            function_definition_with_return_value(p);//有返回值函数定义
        }
        else if((strcmp(store[now].type,"VOIDTK") == 0 && strcmp(store[now+1].type,"IDENFR") == 0))
        {
    ...
    run = 1;
    def = 0;
    if((strcmp(store[now].type,"VOIDTK") == 0 && strcmp(store[now+1].type,"MAINTK") == 0))
...
void principal_function()//＜主函数＞::=void main'('')''{'＜复合语句＞'}'
{
    global_flag = 0;
    void_ret = 1;
    if(strcmp(store[now].type,"VOIDTK") == 0)
    {
        getsym();
        if(strcmp(store[now].type,"MAINTK") == 0)
        {
            getsym();
            ...
            {
                getsym();
                if(strcmp(store[now].type,"RPARENT") != 0) error_l();//错误l
                else getsym();
...
void function_definition_without_return_value(int value[])//<无返回值函数定义>::=void＜标识符>'('<参数表>')''{'<复合语句>'}'
{
    ...
    if(def == 1)
    {
        ...
        {
            getsym();
            if(strcmp(store[now].type,"IDENFR") == 0)
            {
                for(i=0; i<strlen(store[now].word); i++)
...
void sentence()//＜语句＞::=＜循环语句＞｜＜条件语句＞|＜有返回值函数调用语句＞;|＜无返回值函数调用语句＞;｜＜赋值语句＞;｜＜读语句＞;｜＜写语句＞;｜＜情况语句＞｜＜空＞;|＜返回语句＞;|'{'＜语句列＞'}'
{
    int i = 0, j = 0, sign = 0;
    char cmp[255] = {};
...
void loop_statement()//＜循环语句＞::=while'('＜条件＞')'＜语句＞| for'('＜标识符＞＝＜表达式＞;＜条件＞;＜标识符＞＝＜标识符＞(+|-)＜步长＞')'＜语句＞
{
    int state = run;
    int con = 0;
    int back;
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(strcmp(store[now].type,"SEMICN") != 0) error_k();//错误k
                    else getsym();
                    back = now;
                    con = condition();//条件
                    if(strcmp(store[now].type,"SEMICN") != 0) error_k();//错误k
...
void conditional_statement()//＜条件语句＞::=if'('＜条件＞')'＜语句＞［else＜语句＞］
{
    ...
    int state;
    state = run;
    if(strcmp(store[now].type,"IFTK") == 0)
    {
        getsym();
        if(strcmp(store[now].type,"LPARENT") != 0) error_report();
        else
        {
            ...
            sentence();//语句
            if(state == 1 && ret_valid == 0) run = 1;
            if(strcmp(store[now].type,"ELSETK") == 0)
            {
                getsym();
                if(con == 1)
                    run = 0;
...
void assignment_statement()//＜赋值语句＞::=＜标识符＞{'['＜表达式＞']'}02＝＜表达式＞
{
    int *index;
    int i=0,j=0;
    char type = ' ';
...
void read_statement()//＜读语句＞::=scanf'('＜标识符＞')'
{
    ...
    char temp[30] = {};
    char type = ' ';
    if(strcmp(store[now].type,"SCANFTK") == 0)
    {
        getsym();
        if(strcmp(store[now].type,"LPARENT") != 0) error_report();
        else
...
void writing_statements()//＜写语句＞::=printf'('＜字符串＞,＜表达式＞')'|printf'('＜字符串＞')'|printf'('＜表达式＞')'
{
    if(strcmp(store[now].type,"PRINTFTK") == 0)
    {
        getsym();
        if(strcmp(store[now].type,"LPARENT") != 0) error_report();
        else
        {
            getsym();
            if(strcmp(store[now].type,"STRCON") == 0)
            {
                string();//字符串
                if(strcmp(store[now].type,"COMMA") == 0)
                {
                    if(run == 1 && ret_valid == 0)
                ...
                else if(strcmp(store[now].type,"RPARENT") != 0) error_l();//错误l
                else
                {
                    if(run == 1 && ret_valid == 0)
                        fprintf(fp_result,"%s\n",temp_str);
...
void case_statement()//＜情况语句＞::=switch'('＜表达式＞')''{'＜情况表＞＜缺省＞'}'
{
    int aim = 0;
    int sign = 0;
    if(strcmp(store[now].type,"SWITCHTK") == 0)
    {
        getsym();
        if(strcmp(store[now].type,"LPARENT") != 0) error_report();
        else
...
{
    int return_temp = 0;
    if(strcmp(store[now].type,"RETURNTK") == 0)
    {
        getsym();
        ...
        if(strcmp(store[now].type,"LPARENT") == 0)
        {
            getsym();
            if(have_ret == 1 && strcmp(store[now].type,"RPARENT") == 0 && def == 1)
                error_h();//错误h - 形如return();
            expression();//表达式
            if(run == 1)
                return_temp = temp_val;
            if(have_ret == 1 && ((now_type == 'i' && func_tab[func_num].type != 5) || (now_type == 'c' && func_tab[func_num].type != 6)) && def == 1)
...
int situation_table(int aim)//＜情况表＞::=＜情况子语句＞{＜情况子语句＞}
{
    ...
    do
    {
        ...
        if(flag == 1)
            sign = 1;
    }while(strcmp(store[now].type,"CASETK") == 0);
    fprintf(fp_out,"<情况表>\n");
    return sign;
}
int case_sub_statement(int aim)//＜情况子语句＞::=case＜常量＞：＜语句＞
{
    ...
    int sign = 0;
    int type;
    if(strcmp(store[now].type,"CASETK") == 0)
    {
        getsym();
        constant(switch_type);//常量
        if(aim == temp_val)
            sign=1;
        if(strcmp(store[now].type,"COLON") != 0) error_report();
        else
        {
            getsym();
            if(sign != 1)
...
{
    int state = run;
    if(strcmp(store[now].type,"DEFAULTTK") != 0) error_p();
    else
    {
        getsym();
        if(strcmp(store[now].type,"COLON") != 0) error_report();
...
void value_parameter_table(int value[])//＜值参数表＞::=＜表达式＞{,＜表达式＞}｜＜空＞
{
    ...
    int flag = 0;
    int val_num = 0;
    if(strcmp(store[now].type,"RPARENT") != 0 && strcmp(store[now].type,"SEMICN") != 0)//＜空＞
    {
        expression();//表达式
    ...
    if(func_label != -1 && strcmp(use_id,func_tab[func_label].id)!=0 && flag == 0)
        error_e();//错误e
    fprintf(fp_out,"<值参数表>\n");
}
void step()//＜步长＞::=＜无符号整数＞
{
    signed_integer();//无符号整数
    fprintf(fp_out,"<步长>\n");
}
int condition()//＜条件＞::=＜表达式＞＜关系运算符＞＜表达式＞
{
    ...
    if(now_type != 'i')
        error_f();//错误f
    if(strcmp(store[now].type,"LSS") != 0 && strcmp(store[now].type,"LEQ") != 0 &&
       strcmp(store[now].type,"GRE") != 0 && strcmp(store[now].type,"GEQ") != 0 &&
       strcmp(store[now].type,"EQL") != 0 && strcmp(store[now].type,"NEQ") != 0) error_report();
    else
    {
        if(strcmp(store[now].type,"LSS") == 0)//<
            opr = 1;
        else if(strcmp(store[now].type,"LEQ") == 0)//<=
            opr = 2;
        else if(strcmp(store[now].type,"GRE") == 0)//>
            opr = 3;
        else if(strcmp(store[now].type,"GEQ") == 0)//>=
            opr = 4;
        else if(strcmp(store[now].type,"EQL") == 0)//==
            opr = 5;
        else if(strcmp(store[now].type,"NEQ") == 0)//!=
            opr = 6;
        getsym();
        expression();//表达式
        right = temp_val;
...
void expression()//＜表达式＞::=［＋｜－］＜项＞{＜加法运算符＞＜项＞}
{
    ...
    if(minu_flag == 1)
        temp_val = -temp_val;
    while(strcmp(store[now].type,"PLUS") == 0 || strcmp(store[now].type,"MINU") == 0)
    {
        if(strcmp(store[now].type,"PLUS") == 0)
            opr = 1;
        else
...
void term()//＜项＞::=＜因子＞{＜乘法运算符＞＜因子＞}
{
    ...
    int opr = 0;
    factor();//因子
    while(strcmp(store[now].type,"MULT") == 0 || strcmp(store[now].type,"DIV") == 0)
    {
        if(strcmp(store[now].type,"MULT") == 0)
            opr = 1;
        else
            opr = 2;
        getsym();
...
void factor()//＜因子＞::=＜标识符＞｜＜标识符＞'['＜表达式＞']'|＜标识符＞'['＜表达式＞']''['＜表达式＞']'|'('＜表达式＞')'｜＜整数＞|＜字符＞｜＜有返回值函数调用语句＞
{
    int *index;
    int i=0, j=0;
    int temp_ind = 0;
    ...
    int flag = 0;//是否找到相同函数名的标志
    char use_name[255] = {};//函数名小写形式
    if(strcmp(store[now].type,"IDENFR") == 0 && strcmp(store[now+1].type,"LPARENT") != 0)
    {
        error_c(2);//错误c分析
    ...
    else if(strcmp(store[now].type,"LPARENT") == 0)
    {
        getsym();
        expression();//表达式
        if(strcmp(store[now].type,"RPARENT") != 0) error_l();//错误l
    ...
    else if(strcmp(store[now].type,"PLUS") == 0 || strcmp(store[now].type,"MINU") == 0 || strcmp(store[now].type,"INTCON") == 0)
    {
        integer();//整数
        now_type = 'i';
    }
    else if(strcmp(store[now].type,"CHARCON") == 0)
    {
        now_type = 'c';
...
void parameter_table(int value[])//<参数表>::=<类型标识符><标识符>{,<类型标识符><标识符>}|<空>
{
    int val_num = 0;
    int type;
    int id_num = 0;
...
void signed_integer()//<无符号整数>::=<数字>｛<数字>｝
{
